<x-admin-layout>
    <div class="flex h-screen bg-gradient-to-br from-[#ffe5ec] to-[#b8c6db]">
        <div class="flex-1 flex items-start justify-center py-12">
            <div class="w-full max-w-5xl mx-auto bg-[#fff1f1] rounded-xl shadow-lg p-6 mt-12 border border-[#ffd6e0]">
                ...
                <table id="successful_transactions_list" class="min-w-full w-full table-auto rounded-lg overflow-hidden custom-table">
                    <thead>
                        <tr>
    ...
    <style>
        #successful_transactions_list tbody tr:nth-child(even) { background-color: #ffe5ec !important; }
        #successful_transactions_list tbody tr:nth-child(odd) { background-color: #fff1f1 !important; }
        #successful_transactions_list tbody tr { color: #3d405b !important; border-bottom: 1px solid #ffd6e0 !important; }
        #successful_transactions_list tbody tr:hover { background-color: #ffd6e0 !important; transition: background 0.2s; }
        #successful_transactions_list th, #successful_transactions_list td { border-radius: 6px; border: 1px solid #ffd6e0; }
        #successful_transactions_list { border-radius: 12px; overflow: hidden; background: transparent; }
    ...
    <script>
        $(document).ready(function() {
            $('#successful_transactions_list').DataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                ajax: {
                    url: "{{ route('admin.successful-transactions') }}",
                },
                columns: [
                ...
                drawCallback: function() {
                    $('#successful_transactions_list').addClass('table-auto w-full');
            ...
            // Row click for edit
            $('#successful_transactions_list tbody').on('click', 'tr', function(e) {
